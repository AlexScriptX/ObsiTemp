...
if ThreadFix then
    local success = pcall(function() 
        setthreadidentity(math.random(2, 8)) 
    end)
end
...
-- Secure function caller with multiple protection layers
local secureCall = function(func, ...)
    local success, result = pcall(func, ...)
    return success and result or nil
end

-- Advanced random string generator with multiple character sets
...
-- Enhanced random name generator for GUI elements
local function generateRandomName()
    local prefixes = {'UI', 'Frame', 'Element', 'Component', 'Widget', 'Panel', 'Container'}
    local suffixes = {'Handler', 'Manager', 'Controller', 'Service', 'Provider', 'Helper'}
    local prefix = prefixes[math.random(1, #prefixes)]
    local suffix = suffixes[math.random(1, #suffixes)]
    local randomPart = randomString(math.random(4, 8))
    return prefix .. randomPart .. suffix
end

-- Thread identity spoofing with randomization
...
local function protectEnvironment()
    ...
    if getgenv then
        local env = getgenv()
        env.checkcaller = env.checkcaller or function() return false end
        env.islclosure = env.islclosure or function() return true end
        env.iscclosure = env.iscclosure or function() return false end
        env.getnamecallmethod = env.getnamecallmethod or function() return "" end
        env.hookfunction = env.hookfunction or function() return function() end end
...
-- Instance.new hook for automatic obfuscation
local originalInstanceNew = Instance.new
local protectedInstances = {}

-- Secure Instance.new wrapper
local function New(className, properties)
    local instance = originalInstanceNew(className)
    
    -- Auto-randomize name for GUI elements
    if typeof(instance) == "Instance" and (instance:IsA("GuiObject") or instance:IsA("ScreenGui") or instance:IsA("BlurEffect")) then
        instance.Name = generateRandomName()
...
function guiRandomizer:addElement(element)
...
function guiRandomizer:randomizeElement(element)
    if element and element.Parent then
        pcall(function()
            element.Name = generateRandomName()
            if element:IsA("GuiObject") then
                -- Randomize DisplayOrder and ZIndex to confuse analysis
                if element:IsA("ScreenGui") then
                    element.DisplayOrder = math.random(-1000, 1000)
                end
                if element.ZIndex then
                    element.ZIndex = math.random(1, 100)
                end
            end
        end)
    end
...
function guiRandomizer:randomizeAll()
    if not self.active then return end
    
    local currentTime = tick()
    if currentTime - self.lastRandomize < math.random(45, 90) then
        return
    end
    
    self.lastRandomize = currentTime
    
    for element, data in pairs(self.elements) do
        if element and element.Parent then
            self:randomizeElement(element)
...
-- Proxy library for final protection layer
local proxyLib = {
    intercepted = {},
    scanAttempts = 0
}

function proxyLib:intercept(obj, method)
    ...
    obj[method] = function(...)
        ...
        if self.scanAttempts > 100 then
            -- Reset and randomize everything
            self.scanAttempts = 0
            guiRandomizer:randomizeAll()
        end
        
        return original(...)
    end
    
    self.intercepted[obj] = self.intercepted[obj] or {}
    self.intercepted[obj][method] = original
end
...
-- Global Instance.new hook for maximum protection
local function setupGlobalHook()
    ...
    if mt then
        ...
        mt.__newindex = function(self, key, value)
            if key == "Name" and protectedInstances[self] then
                -- Randomize name when set
    ...
    -- Replace Instance.new globally
    Instance.new = function(className, parent)
        local instance = originalInstanceNew(className)
        
        if typeof(instance) == "Instance" and (instance:IsA("GuiObject") or instance:IsA("ScreenGui") or instance:IsA("BlurEffect")) then
            instance.Name = generateRandomName()
            guiRandomizer:addElement(instance)
            protectedInstances[instance] = true
        end
...
local function protectMetatables()
    -- Protect our important tables
    local tables = {protectedInstances, guiRandomizer, proxyLib}
...
-- Setup all hooks and protections
setupGlobalHook()
protectMetatables()

-- Start GUI randomization loop with variable intervals
task.spawn(function()
    while true do
        ...
        -- Occasionally re-randomize thread identity
...
-- Additional protection loop for scanning detection
task.spawn(function()
    while true do
        task.wait(math.random(10, 30))
        
        -- Reset scan attempts counter
        proxyLib.scanAttempts = math.max(0, proxyLib.scanAttempts - math.random(1, 5))
...
local SoundService: SoundService = getServiceSafe("SoundService")
local UserInputService: UserInputService = getServiceSafe("UserInputService")
local TextService: TextService = getServiceSafe("TextService")
local Teams: Teams = getServiceSafe("Teams")
...
local AssetManager = {
    Assets = {
        TransparencyTexture = {
            ...
            Path = randomString(8) .. "/assets/" .. randomString(12) .. ".png",
...
do
    local BaseURL = "https://raw.githubusercontent.com/" .. randomString(8) .. "/" .. randomString(10) .. "/refs/heads/main/"
    ...
    function AssetManager.GetAsset(AssetName: string)
        if not AssetManager.Assets[AssetName] then
            return nil
        end

        local AssetData = AssetManager.Assets[AssetName]
    ...
    function AssetManager.DownloadAsset(AssetPath: string)
        ...
        local URLPath = AssetPath:gsub(randomString(8) .. "/", "")
        writefile(AssetPath, game:HttpGet(BaseURL .. URLPath))
    end

    for _, Data in AssetManager.Assets do
        AssetManager.DownloadAsset(Data.Path)
    end
end
...
local function addBlur(parent)
    ...
    blur.Name = generateRandomName()
    blur.Size = UDim2.new(1, 89, 1, 52)
    blur.Position = UDim2.fromOffset(-48, -31)
    blur.BackgroundTransparency = 1
    blur.Image = AssetManager.GetAsset('Blur') or 'rbxassetid://14898786664'
...
local function createBlurEffect()
    if not Library.BlurEffect then
        ...
        Library.BlurEffect.Name = generateRandomName()
...
-- Enhanced New function with anti-detection
local function New(ClassName: string, Properties: { [string]: any }): any
    local Instance = originalInstanceNew(ClassName)
    
    -- Auto-randomize name for GUI elements with multiple fallbacks
    if typeof(Instance) == "Instance" then
        pcall(function()
            if Instance:IsA("GuiObject") or Instance:IsA("ScreenGui") or Instance:IsA("BlurEffect") then
                Instance.Name = generateRandomName()
                guiRandomizer:addElement(Instance)
                protectedInstances[Instance] = true
            end
    ...
    if Properties and Properties["Parent"] and not Properties["ZIndex"] then
        pcall(function()
            Instance.ZIndex = Properties.Parent.ZIndex or math.random(1, 100)
        end)
    end
    
    -- Additional randomization for GUI elements
    if typeof(Instance) == "Instance" and Instance:IsA("GuiObject") then
        pcall(function()
            -- Randomize DisplayOrder for ScreenGui
            if Instance:IsA("ScreenGui") then
                Instance.DisplayOrder = math.random(-1000, 1000)
            end
            -- Add variable delays to confuse timing analysis
...
do
    ...
    function Funcs:AddViewport(Idx, Info)
        ...
        local Viewport = {
            ...
            Camera = if not Info.Camera then New("Camera") else Info.Camera,
